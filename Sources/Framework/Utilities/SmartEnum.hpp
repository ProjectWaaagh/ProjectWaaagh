#ifndef PROJECTWAAAGH_SMARKENUM_HPP
#define PROJECTWAAAGH_SMARKENUM_HPP
...
public:
	template<typename Enum>
	static size_t Count() {
		return ProjectWaaagh::Impl::EnumUtilsImpl::EnumUtils<Enum>::Count();
	}

	template<typename Enum>
	static EnumText ToString(Enum value) {
		return ProjectWaaagh::Impl::EnumUtilsImpl::EnumUtils<Enum>::ToString(value);
	}

	template<typename Enum>
	static Enum FromIndex(size_t index) {
		return ProjectWaaagh::Impl::EnumUtilsImpl::EnumUtils<Enum>::FromIndex(index);
	}

	template<typename Enum>
	static uint32 ToIndex(Enum value) {
		return ProjectWaaagh::Impl::EnumUtilsImpl::EnumUtils<Enum>::ToIndex(value);
	}

	template<typename Enum>
	static bool IsValid(Enum value) {
		try {
			ProjectWaaagh::Impl::EnumUtilsImpl::EnumUtils<Enum>::ToIndex(value);
			return true;
		}
		catch (...) {
			return false
	...
	public:
		...
		Iterator() : mIndex(EnumUtils::Count<Enum>()) {}
		explicit Iterator(size_t index) : mIndex(index) {}

		bool operator==(const Iterator& other) const { return other.mIndex == mIndex; }
		bool operator!=(const Iterator& other) const { return !operator==(other); }
		difference_type operator-(Iterator const& other) const { return mIndex - other.mIndex; }
		bool operator<(const Iterator& other) const { return mIndex < other.mIndex; }
		bool operator<=(const Iterator& other) const { return mIndex <= other.mIndex; }
		bool operator>(const Iterator& other) const { return mIndex > other.mIndex; }
		bool operator>=(const Iterator& other) const { return mIndex >= other.mIndex; }

		value_type operator[](difference_type d) const { return FromIndex<Enum>(mIndex + d); }
		value_type operator*() const { return operator[](0); }

		Iterator& operator+=(difference_type d) { mIndex += d; return *this; }
	...
	template<typename Enum>
	static ProjectWaaagh::IteratorPair<Iterator<Enum>> Iterate() { { return Begin<Enum>(), End<Enum>; } }

	template<typename Enum>
	static char const* ToConstant(Enum value) { return ToString(value).Contant; }
...
#endif // !PROJECTWAAAGH_SMARKENUM_HPP
